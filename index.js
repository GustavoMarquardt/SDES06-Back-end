...
initializeDatabase().then(
  ({ Festa, Usuario, Avaliacao, Comentario, sequelize }) => {
    // Rotas de Festa
    router.post("/cadastrarFesta", async (req, res) => {
      try {
        ...
        if (imagem_festa_base64) {
          ...
          const matches = imagem_festa_base64.match(
    ...
    router.get("/listarFestaCriador/:id", async (req, res) => {
      ...
      try {
        ...
        if (festas.length === 0) {
          return res
            .status(404)
            .json({ mensagem: "Nenhuma festa encontrada para este criador." });
        }
    ...
    router.post("/usuario/loginUsuario", async (req, res) => {
      ...
      try {
        ...
        return res
          .status(500)
          .json({ mensagem: "Erro interno ao autenticar usuário." });
      }
    });
    ...
    router.get("/avaliacoes/criador/:id_criador", async (req, res) => {
      try {
        ...
        const resultado = await Avaliacao.listar_avaliacao_por_criador(
          id_criador
        );
        res.status(resultado.status).json(resultado);
      } catch (error) {
    ...
    router.post("/comentarios", async (req, res) => {
      try {
        ...
        if (!id_avaliacao || !id_criador_comentario || !comentario) {
          return res
            .status(400)
            .json({ mensagem: "Faltando dados obrigatórios" });
        }
    ...
    router.get("/comentarios/:id_avaliacao", async (req, res) => {
      ...
      try {
        const comentarios = await Comentario.listar_comentarios_por_festa(
        ...
        return res
          .status(500)
          .json({ message: "Erro ao listar comentários por festa" });
      }
    });
    ...
    router.get("/comentarios/criador/:id_criador", async (req, res) => {
      try {
        const comentarios = await Comentario.listar_comentarios_por_criador(
          req.params.id_criador
        );
        return res.status(comentarios.status).json(comentarios);
      } catch (error) {
        ...
        return res
          .status(500)
          .json({ message: "Erro ao listar comentários por criador" });
      }
    });
    ...
    router.put("/comentarios/:id", async (req, res) => {
      try {
        ...
        return res
          .status(comentarioAtualizado.status)
          .json(comentarioAtualizado);
      } catch (error) {
        console.error("Erro ao atualizar comentário:", error);
        return res
          .status(500)
          .json({ message: "Erro ao atualizar comentário" });
      }
    });
    ...
    router.delete("/comentarios/:id", async (req, res) => {
      try {
        const comentarioExcluido = await Comentario.excluir_comentario(
          req.params.id
        );
    ...
    app.put("/comentarios/:id", (req, res) => {
      ...
      const comentarioAtualizado = req.body;
      const query = "UPDATE comentarios SET comentario = ? WHERE id = ?";
      db.query(
        query,
        [comentarioAtualizado.comentario, id],
        (error, result) => {
          if (error) {
            console.error(error);
            return res.status(500).send("Erro ao atualizar o comentário.");
          }
